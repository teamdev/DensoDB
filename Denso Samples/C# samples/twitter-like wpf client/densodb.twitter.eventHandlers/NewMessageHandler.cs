...
namespace densodb.twitter.eventHandlers
{
  ...
  [DeNSo.Meta.HandlesCommand(DensoBuiltinCommands.Set)]
  [Export(typeof(ICommandHandler))]
  public class MessageHandler: DeNSo.Core.EventHandlers.BaseCommandHandler
  {

    public override void OnHandle(IStore store, string collection, BSonDoc command, BSonDoc document)
    {
      ...
      var userprofile = store.GetCollection("users").Where(d => d["UserName"].ToString() == message.From).FirstOrDefault().FromBSon<UserProfile>();
