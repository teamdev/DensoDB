...
namespace DeNSo.Core.Filters
{
  ...
  {
    // 128 bit output, 64 bit platform versio
    ...
    private static void MixBody(ulong k1, ulong k2, ref ulong h1, ref ulong h2)
    {
      h1 ^= MixKey1(k1);
    ...
    private static byte[] ProcessBytes(byte[] bb)
    {
      ulong h1 = 0L;
      ulong h2 = 0L;
      ulong length = 0L;
      ...
      while (remaining >= READ_SIZE)
      {
        ...
        MixBody(k1, k2, ref h1, ref h2);
      }

      // if the input MOD 16 != 0
      if (remaining > 0)
        ProcessBytesRemaining(bb, remaining, pos, ref h1, ref h2, ref length);

      return Hash(ref h1, ref h2, ref length);
    }

    private static void ProcessBytesRemaining(byte[] bb, ulong remaining, int pos, ref ulong h1, ref ulong h2, ref ulong length)
    {
      ulong k1 = 0;
      ulong k2 = 0;
      length += remaining;
    ...
    private static byte[] Hash(ref ulong h1, ref ulong h2, ref ulong length)
    {
      h1 ^= length;
      h2 ^= length;
